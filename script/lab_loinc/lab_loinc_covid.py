...
def main():
    ...
    fsh_included_lab_tests = "\n".join([f'* $loinc#{l} "{loinc_display_names[l]}"' for l in included_lab_tests])
    fsh = f'''//
//
// WARNING!
...
'''
    ...
    intro = '''This value set includes a subset of the LOINCs found at <{LOINC_COVID_URL}> that identify COVID-19-related laboratory tests. Only those laboratory tests that include qualitative results we believe to be useful in SMART Health Cards are included.
...
'''
    ...
    # Write FSH to file
    filename = os.path.join(dirname, '../../input/fsh/covid-lab-loinc.fsh')
...
'''
    ...
    # Write FSH to file
    filename = os.path.join(dirname, '../../input/fsh/lab-qualitative-results.fsh')
    with open(filename, 'w') as f:
        f.write(fsh)


    intro = f'''Whenever possible, SNOMED CT codes SHALL be be used. If no equivalent code is available, LOINCs MAY be used as a fallback. 
